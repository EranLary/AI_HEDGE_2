...
**Per-PR site previews.** [.github/workflows/preview-site.yml](.github/workflows/preview-site.yml) creates `pr-<N>-hedge-in-a-box-site.fly.dev` for any PR that touches `frontend/**`, `Dockerfile.site`, or `fly.site.toml`. The preview's `/data` volume is forked from the latest prod snapshot at PR open and kept for the life of the PR (staleness accepted). The preview's DB is a Neon branch named `pr-<N>` off `main` in the `ai-hedge` Neon project — previews can mutate freely without touching prod. Machines auto-stop when idle. The Fly app, volume, and Neon branch are destroyed when the PR closes; [.github/workflows/preview-site-cleanup.yml](.github/workflows/preview-site-cleanup.yml) is a daily safety net that nukes both Fly preview apps and orphaned Neon `pr-*` branches older than 14 days.
