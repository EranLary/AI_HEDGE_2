...
async function loadHistoricalDashboards(workspace: Workspace): Promise<LoadedDashboard[]> {
  ...
  try {
    ...
    for (const r of dbRows) {
      const row = {
        ...
        payload: normalizePayload(String(r.ticker || "").toUpperCase(), r.dashboard as DashboardPayload, { reportMtime: new Date(r.generated_at).toISOString() }),
      };
  ...
  for (const entry of listDashboardReports()) {
    ...
    const row = { ticker, generatedAt, payload: normalizePayload(ticker, payload, { reportId: entry.report_id || undefined, reportFile: entry.path, reportMtime: new Date(entry.mtimeMs).toISOString() }) };
