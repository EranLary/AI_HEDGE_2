...
export async function buildDiscoveryPayload(params: {
  lensType?: string | null;
  lensKey?: string | null;
} = {}): Promise<DiscoveryPayload> {
  const requestedLensType = normalizeLensType(params.lensType);
  const requestedLensKey = cleanLensKey(params.lensKey);
