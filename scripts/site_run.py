...
DEFAULT_LLM_TOTAL_ESTIMATE = 50


def _estimate_total_llm_calls() -> int:
    raw = str(
        os.getenv("SITE_RUN_LLM_TOTAL_ESTIMATE", str(DEFAULT_LLM_TOTAL_ESTIMATE))
        or DEFAULT_LLM_TOTAL_ESTIMATE
    ).strip()
    try:
        n = int(raw)
        return max(1, n)
    except Exception:
        return DEFAULT_LLM_TOTAL_ESTIMATE
